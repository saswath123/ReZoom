...
def generate_worst_resume_template(data, gap_analysis, output_path):
    ...
    add_watermark_to_image(img, color=(170, 180, 195), y_offset=50)
...
def generate_poor_resume_template(data, gap_analysis, output_path):
    ...
    add_watermark_to_image(img, color=(170, 180, 195), y_offset=40)
    img.save(output_path, "PNG", dpi=(300, 300))
    return output_path
...
def add_watermark_to_image(img, color=(170, 180, 195), y_offset=0):
    """Draw a professional watermark on the bottom right of the image as normal text in light color"""
    draw = ImageDraw.Draw(img)
    ...
    watermark_text = f"Generated by TalentLens AI Resume Intelligence Platform on {date_str} at {time_str} IST"
    
    # Use a small size (e.g. 14) for high resolution 1600x1200 image
    font_size = 14
    try:
    ...
    y = height - text_height - margin - y_offset
...
def generate_professional_resume_template(data, gap_analysis, output_path):
    ...
    add_watermark_to_image(img, color=(170, 180, 195))
    img.save(output_path, "PNG", dpi=(300, 300))
    return output_path